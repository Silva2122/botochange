...
💸Доход с рефералов 1 Уровня - 8%
💸Доход с рефералов 2 Уровня - 2%'''.format(referals, refs2nd, bot_info.username, user['user_id']),
...
💸Доход с рефералов 1 Уровня - 8%
💸Доход с рефералов 2 Уровня - 2%'''.format(user['referal'], referals, refs2nd, bot_info.username, user['user_id'], bot_info.username, user['user_id']),
                    'markup': messages.start['markup']
                }
                try:
...
💸Доход с рефералов 1 Уровня - 8%
💸Доход с рефералов 2 Уровня - 2%'''.format(
...
💸Доход с рефералов 2 Уровня - 2%'''.format(
    ...
    # Статистика todo в статистике ебнуть всю статистику проекта А именно: пользователей всего,пользователей за сегодня, постов всего,Заработаннт всего Выплачено всего
        ...
        return send_message(message, messages.rules)
    ...
    '''.format(message.from_user.id, len(usr_chn), view_count, view_bal, post_bal, round(ref_pay, 2), round(view_bal + post_bal + round(ref_pay, 2), 2),
        ...
        👥 Реферальный баланс: {}р'''.format(round(user['balance'] + user['ref_balance'], 2), round(user['balance'], 2), round(user['ref_balance'], 2)),
                 'markup': [['💸 Вывод', '💰 Пополнение'], ['◀️ Вернуться']]}
    ...
    if text == 'Другой способ':
        return send_message(message, {'text': 'Если вы хотите пополнить баланс другим способом - напишите администратору @guldq', 'markup': messages.start['markup']})
    if text == 'Мои заказы':
...
logger.info(s)
logger.info(tb.set_webhook(
            url=s,
#            certificate=open(settings.WEBHOOK_SSL_PRIV, 'r'),
            allowed_updates=['update_id','message','edited_message','channel_post','edited_channel_post','inline_query','chosen_inline_result','callback_query','shipping_query','pre_checkout_query']))
